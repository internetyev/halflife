...
- [ ] `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` set to the registered Plausible site (matches the final domain). Plausible site created under the human's account; **register two custom-event goals**: `form-submit` (fired by `app/page.tsx` after a successful `/api/analyze`, with a `cache: HIT|MISS` prop so the dashboard can split paid Claude calls from free KV-cached ones — the single most useful split for the PLAN.md p50-cost-per-result KPI) and `share-click` (fired by `components/share-buttons.tsx`, with `channel: linkedin|x|copy` + `slug` props so the dashboard reads which primitive is actually shared and against which role). Both are wired in code as of L5.25 (`script.tagged-events.js` + `lib/analytics/plausible.ts` `trackEvent()`); the dashboard goals just need to exist on the Plausible site so the events surface as named goals instead of generic custom events.
...
- [x] Methodology link in footer points at `/docs/methodology` or the rendered equivalent — a user who wants to argue with the number must be able to read the rubric. (L5.14: `components/site-footer.tsx` carries a Methodology link to the GitHub-rendered `docs/methodology.md` — the "rendered equivalent" half of this gate's wording, because there is no in-app methodology route. An in-app Markdown rendering of methodology would be a separate, larger leaf and is deferred.)
- [x] Privacy note at `/privacy` (`app/privacy/page.tsx`, L5.13) **and footer link** (`components/site-footer.tsx`, L5.14): Plausible is cookieless, no PII collected, role inputs are cached for 30 days keyed by slug. No GDPR cookie banner required (D-012, PLAN.md). The page lists Anthropic / Vercel KV / Plausible / Plunk as the only sub-processors and a `privacy@<domain>` contact alias the human owns (the host literal `halflife.work` will swap with the L1.7b/L5.1 pick in one edit). `/privacy` is in the sitemap, indexable; the shared footer (L5.14) renders on every route via `app/layout.tsx` and points at it, so the page is discoverable from any page (the global-error frame correctly opts out per D-039 — its own `<html>`/`<body>` and no `next/link` because client routing may be compromised).
