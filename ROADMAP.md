...
_Last updated: 2026-05-13 (L2.8 share buttons — `components/share-buttons.tsx` (client component) wired into both `app/page.tsx` and `app/role/[slug]/page.tsx`. LinkedIn-first, then X (Twitter), then copy-link. Share target is always the canonical `/role/<slug>` URL so OG image + metadata travel with the link.)_
